...
BEGIN
    ...
    SELECT public.calculate_producer_balances_simple(p_producer_id) INTO v_balances_result;
    ...
    SELECT COALESCE(json_agg(sales_row), '[]'::json) INTO v_sales_history FROM (
        ...
        WHERE p.producer_id = p_producer_id OR (p.id IS NULL AND s.product_id IN (
            SELECT DISTINCT product_id FROM public.sales WHERE product_id NOT IN (SELECT id FROM public.products)
        ))
