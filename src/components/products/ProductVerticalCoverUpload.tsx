...
export const ProductVerticalCoverUpload = ({ onUploadSuccess, initialUrl = '', userId }: ProductVerticalCoverUploadProps) => {
  ...
  if (uploading) return <div className="border-2 border-dashed rounded-lg p-8 h-[200px] flex items-center justify-center"><Loader2 className="h-10 w-10 animate-spin" /></div>;
  if (uploadedUrl) return <div className="relative w-36 h-64 border rounded-lg group"><img src={uploadedUrl} alt="Capa Vertical" className="w-full h-full object-cover rounded-lg" /><div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity rounded-lg"><Button type="button" variant="destructive" size="icon" onClick={handleRemoveImage}><X className="h-5 w-5" /></Button></div></div>;
  
  return (
    <div {...getRootProps()} className={`border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition-colors ${ isDragActive ? 'border-primary bg-primary/10' : 'border-border' } h-[200px]`}>
      <input {...getInputProps()} />
